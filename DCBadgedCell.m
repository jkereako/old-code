...
#import "DCBadgedCell.h"
@implementation DCBadgeView

@synthesize _width, _badgeNumber, _parent, _badgeColor, _badgeColorHighlighted;
// from private
@synthesize _font;
...
- (id) initWithFrame:(CGRect) frame {
	...
	_font = [[UIFont boldSystemFontOfSize: 14] retain];
...
- (void) drawRect:(CGRect)rect {	
	...
	CGSize numberSize = CGSizeMake(0, 0);	
	CGRect bounds = CGRectMake(0, 0, 0, 0);
	CGContextRef context = UIGraphicsGetCurrentContext();
	float radius = 0.0;
	UIColor *color = nil;
	
	countString = [[NSString alloc] initWithFormat: @"%d", self.badgeNumber];
	numberSize = [countString sizeWithFont: _font];
	bounds = CGRectMake(0 , 0, numberSize.width + 16 , 18);
	radius = bounds.size.height / 2.0;
	
	_width = numberSize.width + 16;
	...
	if (_parent.highlighted || _parent.selected) {
		if (self.badgeColorHighlighted)
			color = self.badgeColorHighlighted;
		else
			color = [UIColor colorWithRed:1.0 green:1.0 blue:1.0 alpha:1.000];
	}
		...
		if (self.badgeColor)
			...
			color = [UIColor colorWithRed:0.530 green:0.600 blue:0.738 alpha:1.000];
	}
...
- (void) dealloc {
	...
	SAFE_RELEASE(_badgeColorHighlighted);
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        // Initialization code
		badge = [[DCBadgeView alloc] initWithFrame:CGRectZero];
		badge._parent = self;
