...
#import "DCSplashView.h"

@implementation DCSplashView
...
- (id)initWithImage:(UIImage *)screenImage {
	...
	if (self = [super initWithFrame:[[UIScreen mainScreen] applicationFrame]]) {
		...
		delay = 2;
		touchAllowed = NO;
		animation = DCSplashViewAnimationNone;
		animationDelay = .5;
		isFinishing = NO;
	}
	return self;
...
- (void)dismissSplash {
	...
	if (self.isFinishing || self.animation == DCSplashViewAnimationNone) {
		[self dismissSplashFinish];
	} 
	
	else if (self.animation == DCSplashViewAnimationSlideLeft) {
	...
	else if (self.animation == DCSplashViewAnimationFade) {
